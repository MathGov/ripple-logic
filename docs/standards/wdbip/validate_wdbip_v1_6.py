...
def validate(record_path: Path, schema_path: Path):
    ...
    if data.get("protocol_version") != "WDBIP v1.6":
        fail("protocol_version must be WDBIP v1.6")
    if data.get("schema_version") != "wdbip_record_v1_6":
        fail("schema_version must be wdbip_record_v1_6")
    ...
    if data.get("canon_version") != "RippleLogic v12.6":
        fail("canon_version pin mismatch for this standalone v1.6 package")
    ...
    for idx, token in enumerate(tokens):
        ...
        # Canon/WDBIP D6-D7 rights-routing binding.
        if token.get("primary_dimension") in ("D6_MEANING", "D7_ENVIRONMENT"):
            rr = token.get("rights_routing")
            if not isinstance(rr, dict):
                fail(f"RIGHTS_RELEVANT_EFFECT_UNROUTED {tid}: D6/D7 effect requires rights_routing review")
            else:
                status = rr.get("status")
                if status == "RIGHTS_RELEVANT_ROUTED" and not rr.get("rights_record_ref"):
                    fail(f"RIGHTS_RELEVANT_EFFECT_UNROUTED {tid}: routed effect requires rights_record_ref")
                elif status == "RIGHTS_RELEVANT_EXTENSION" and not rr.get("extension_ref"):
                    fail(f"RIGHTS_RELEVANT_EFFECT_UNROUTED {tid}: extension requires extension_ref")
                elif status in ("NOT_RIGHTS_RELEVANT_REVIEWED", "RIGHTS_RELEVANT_NON_MATERIAL_REVIEWED") and not rr.get("review_rationale"):
                    fail(f"RIGHTS_RELEVANT_EFFECT_UNROUTED {tid}: reviewed non-routing status requires review_rationale")
                elif status not in ("NOT_RIGHTS_RELEVANT_REVIEWED", "RIGHTS_RELEVANT_ROUTED", "RIGHTS_RELEVANT_EXTENSION", "RIGHTS_RELEVANT_NON_MATERIAL_REVIEWED"):
                    fail(f"RIGHTS_RELEVANT_EFFECT_UNROUTED {tid}: invalid rights_routing status")
...
def main():
    if len(sys.argv) not in (2, 3):
        print("usage: validate_wdbip_v1_6.py RECORD.json [SCHEMA.json]", file=sys.stderr)
        return 2
    record = Path(sys.argv[1])
    schema = Path(sys.argv[2]) if len(sys.argv) == 3 else Path(__file__).with_name("wdbip_record_v1_6.schema.json")
    errors = validate(record, schema)
    if errors:
        print("WDBIP v1.6 INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("WDBIP v1.6 VALID")
    return 0
