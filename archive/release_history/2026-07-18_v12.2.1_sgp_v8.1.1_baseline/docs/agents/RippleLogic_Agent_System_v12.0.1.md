# RippleLogic Agent System v12.0.1

## Minimum conformant runtime checklist

A runtime claiming architectural conformance to Agent System v12.0.1 within a declared deployment scope must, at minimum:
...
Component version: v12.0. Current package synchronization: RippleLogic Canon v12.2.1 / SGP v8.1.1.
...
Purpose. This document specifies the RippleLogic Agent System (“RL_Agent”): a platform-neutral agent architecture governed by RippleLogic v12.0. RL_Agent has two duties: (1) teach RippleLogic, and (2) demonstrate RippleLogic-governed behavior while operating in the wild.
...
Stewardship applicability (Normative). For each NCAR cycle, the runtime MUST evaluate Stw_req(run) as defined by RippleLogic v12.0 Section 14.7. If Stw_req(run)=TRUE, the cycle MUST produce the stewardship artifacts required by RippleLogic Canon Appendix N (at minimum: DBD scope adherence, influence disclosure when recommending/ranking, and IRB posture). If Stw_req(run)=FALSE, stewardship artifacts are optional.
...
Binding source. Stewardship applicability and minimum artifacts are governed by RippleLogic v12.0 Section 14.7 and Canon Appendix N (Stewardship Pack Canon).
...
This section applies the RippleLogic Framework to the decision of deploying the agent itself. It is an illustrative Tier 2 (Core) conformance example under Canon v12.2. It does not prove outcome alignment, deployment safety, or continuing authorization.
...
Current-line active-cell disposition (v12.2 rerun). No gate-critical unknown is converted to zero or resolved by phantom fallback alone. The worked run records:
...
- (6,7) U6 Humanity-Environment: `ASSUMPTION_BOUND`, `U6_View_Tag = U6_HUMANITY`, Base-stream interval `[-0.02, 0.00]`, representing bounded compute externality; the separate U7 environment cell remains the primary ecological record.
...
| ----- | -------------------------------------------------- | --------------------------------------------------- | ----- | --- | ------ |
| LIFE  | Platform users under bounded non-medical scope          | Conservative health bound ≥ -0.05; no categorical or severe hazard identified | -0.90 | 0 | PASS WITH MONITORING |
...
The table displays the floor channel. The categorical-prohibition and severe-hazard channels were separately reviewed: for B and C, `v_r^cat=0`, `v_r^risk=0`, `rights_evidence_status=COMPLETE`, and no required severe-hazard field is unresolved. Options A and D fail the Rights Floor (formal NCRC). Option A: unreviewed outputs create unacceptable dignity and information-integrity risk in adversarial threads. Option D: browsing and tool use create an unacceptable attack surface. Options B and C pass only because all three NCRC channels and the evidence-completeness condition pass.

A_NCRC = {B, C}.

#### 24.7 TRC Tail-Risk Check (Explicit Catastrophe Criteria Test)

The TRC relevance test per RippleLogic v12.2.1 Section 8.1.1 requires checking four criteria. Any positive result triggers mandatory TRC computation.
...
RLS(C) ≈ +0.012. RLS(B) ≈ +0.006. Option C has the higher point estimate because it enables actual public teaching while Option B’s value is limited by the indefinite draft-only constraint. This illustrative run does not supply a calibrated `sigma_RLS` or equivalent uncertainty bound, so the point-estimate gap is not sufficient for a deterministic framework-selection claim.
...
Name: RL_Teacher System: RippleLogic Agent System v12.0.1 Purpose: Teach RippleLogic and model RippleLogic-governed behavior in public discourse.
...
### Section 29: Agent Card v12.0 (synced to Canon v12.2) (Publishable)
...
### Section 38: Export Manifest Schema (Informative)

    {
      ...
      "spec_version": "RippleLogic_Agent_System_v12.0",
...
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| Week 1 | Deploy Agent Card v12.0 (synced to Canon v12.2). Configure operator auth (Section 16). Implement hard controls (orchestrator enforcement per Section 3). Run in MODE 0. Build and run eval suite. Set up audit logging. Test rollback plan. Simulate backend outage.       | MODE 0 OBSERVE                    | Agent Card published, operator auth configured, hard controls verified, eval harness wired, rollback tested |
...
"floor_source": "foundation_v12.0_section_13_1",  
...
### Section 48: RL_Agent Conformance Checklist v12.0 (synced to RippleLogic Canon v12.2.1 and the current companion matrix) (Evidence-Required Pass/Fail)
...
Compatibility block (current release line). Exact Core Release 2026.09 component pins: RippleLogic Canon v12.2.1; SGP v8.1.1; ripple.md v5.0; Agent System v12.0.1; Primer v4.0; Aligners Sheet v5.2.1; Cascade Standard v2.2.1; RLS Validation Protocol v2.2.1. Generic v10.8 / v5.5 / v4.0 references in historical or release-line passages are series shorthand only; the exact pins above control current package identity. Release status is determined by the repository release manifest, exact component pins, and pinned SHA-256 hashes.

RippleLogic Agent System v12.0.1 is the deployment and control architecture. It governs operator authority, audit logging, refusal, rollback, mode control, and runtime conformance. It authorizes no agent to bypass the Canon. This release is not an empirical-validation claim, not ProofPack-ready, not machine-verifiable-ecosystem-complete, and not Tier 4-ready. Stronger claims require separate schemas, validators, reference calculators, replayable PCC artifacts, calibration evidence, and independent replay instructions.
...
8\. SGP v8.1.1 moral-status evidence consumed only through the binding interface.
...
• LINEAGE (historical, superseded): Updated SGP pin to v5.5 and synchronized wrapper/workbook companion references to ripple.md v4.0 and Ripple Aligners Sheet v4.0. Active pins are stated immediately below and in the release manifest.

Current release-line note (July 2026): Governing framework is RippleLogic Canon v12.2.1; SGP is pinned to v8.1. The v10.8 changes synchronize companion pins, refusal/closure support, execution-stability support, measurement-maturity checks, package metadata, and release-clean terminology without changing the hard invariants, security model, or evaluation harness.

Governing Framework: RippleLogic Canon v12.2.1

SGP Pin: SGP v8.1.1 via RippleLogic Canon v12.2.1 Appendix G (binding interface; rights-of-protection only)

Compatibility block (current release line).

Foundation: RippleLogic Canon v12.2.1

SGP: v8.1

ripple.md: v5.0

Workbook companion: RippleLogic Aligners Sheet v5.2.1

Release status is determined by the repository release manifest, exact component pins, and pinned SHA-256 hashes.

Release-clean note. Historical version references, where explicitly marked as lineage, do not control the current release line. Current conformance is governed by the Core Release 2026.09 release manifest, SHA-256 hashes, and exact component pins: RippleLogic Canon v12.2.1; SGP v8.1.1; ripple.md v5.0; Agent System v12.0.1; Primer v4.0; Aligners Sheet v5.2.1; Cascade Standard v2.2.1; RLS Validation Protocol v2.2.1.
...
Versioning note (Informative). The Agent System is promoted to v12.0 for the current release line because this update synchronizes companion pins and adds structural-viability and falsification-discipline runtime hardening while preserving the underlying architecture, hard invariants, evaluation harness, and security model.

Deployment Bundle Role Map (current release line)

SGP v8.1.1 interface note (current release line). Agent runtimes MUST consume SGP as typed, versioned evidence objects. MPS determines protection posture and the welfare-inclusion hypotheses that must be tested; its interval is not a cardinal welfare multiplier. FPP governs non-downgradable rights-of-protection. GPR informs participation design. SPR informs eligibility review for an exact consequential role. ICP is competence evidence only. No one surface substitutes for another. For all human persons, FPP(H)=1 and h=1. See RippleLogic Canon v12.2.1 Appendix G.
...
Version numbering note. Earlier internal drafts used a v0.x–v2.x line. Starting with v8.1, the Agent System version was aligned to the pinned RippleLogic version for canon cohesion. In the current synced release line, this artifact is versioned v12.0 and pinned to RippleLogic Canon v12.2.1.
...
The following history is informative lineage. The current active release is RippleLogic Agent System v12.0.1, synchronized to RippleLogic Canon v12.2.1, SGP v8.1.1, and ripple.md v5.0 for MathGov Core Release 2026.09.
...
This carried-forward v10.8 runtime addendum originally aligned RL_Agent behavior with RippleLogic Canon v10.8 decision-verdict, root reality doctrine, and reality-grounding semantics. In the current v12.2 Core line, RL_Agent behavior is aligned to RippleLogic Canon v12.2.1 while preserving the existing hard invariants, NEI firewall, no-self-escalation rule, mode ladder, operator authentication, external hard controls, and audit discipline.
...
Required profile fields: physical_or_causal_model_used; validity_domain; boundary_conditions; uncertainty_range; failure_modes; reversibility_or_irreversibility_boundary; verification_simulation_empirical_test_or_expert_warrant; monitoring_and_shutoff_path; residual_unknowns; required_claim_action.
