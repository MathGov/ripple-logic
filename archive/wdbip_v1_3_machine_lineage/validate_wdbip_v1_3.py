...
def validate(record_path: Path, schema_path: Path):
    ...
    if data.get("protocol_version") != "WDBIP v1.3":
        fail("protocol_version must be WDBIP v1.3")
    if data.get("schema_version") != "wdbip_record_v1_3":
        fail("schema_version must be wdbip_record_v1_3")
    ...
    if data.get("canon_version") != "RippleLogic v12.3":
        fail("canon_version pin mismatch for this standalone v1.3 package")
...
def main():
    if len(sys.argv) not in (2, 3):
        print("usage: validate_wdbip_v1_3.py RECORD.json [SCHEMA.json]", file=sys.stderr)
        return 2
    record = Path(sys.argv[1])
    schema = Path(sys.argv[2]) if len(sys.argv) == 3 else Path(__file__).with_name("wdbip_record_v1_3.schema.json")
    errors = validate(record, schema)
    if errors:
        print("WDBIP v1.3 INVALID")
        for error in errors:
            print(f"- {error}")
        return 1
    print("WDBIP v1.3 VALID")
    return 0
