# Reproducible MathGov Run Walkthrough v4

**Purpose:** Show how a small decision can be recorded under run-record schema v4 so that another person can reproduce the gate states and decision state without reading the entire Canon.

**Files:**

- Human-readable walkthrough: this file
- Machine-readable record: `reusable_cups_run_v4.json`
- Schema: `schemas/mathgov_run_record_v4.schema.json`
...
```bash
python release/VALIDATE_MATHGOV_RUN.py docs/examples/reproducibility/reusable_cups_run_v4.json
```
...
The validator confirms that the record is structurally complete for schema v4 and that its gate, ranking, authority, execution, responsibility-continuity, and configuration-assurance states do not contradict one another. It does not verify evidence truth, legal correctness, empirical instrument validity, physical safety, or decision superiority.
