...
## 1.1 Physical/Causal Admissibility Evidence profile for pilots

For any pilot, worked example, agent workflow, classroom simulation, policy scenario, or product scaffold that involves material physical or causal execution, include the Physical/Causal Admissibility Evidence Profile before selection. Minimum fields are: physical_or_causal_model_used; validity_domain; boundary_conditions; uncertainty_range; failure_modes; reversibility_or_irreversibility_boundary; verification_simulation_empirical_test_or_expert_warrant; monitoring_and_shutoff_path; residual_unknowns; required_claim_action.

The profile is training and audit scaffolding unless independently validated domain instruments are supplied. It belongs inside RG and CSV and does not replace domain engineering, safety cases, clinical review, environmental assessment, or formal verification when those are required.
