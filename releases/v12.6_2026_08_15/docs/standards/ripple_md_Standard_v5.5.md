# **RLS-RFC-0001: The ripple.md Standard v5.5**

## **v5.5 Current Release Integration**
...
|---------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------|
| Critical-path tempo       | Consequence-tempo compatibility is evaluated across the control-critical path and may be concurrent; the standard does not require a simplistic sequential pipeline. | Tempo assurance does not prove physical safety or deployment readiness. |
| Responsibility continuity | Named owner, monitor, intervener, appeal recipient, and post-incident custodian roles remain assigned across the consequence horizon.                                | A wrapper record cannot create legal or institutional authority.        |
| Wrapper boundary          | Non-Domination, Epistemic Integrity, assurance, waiver, appeal, and reconstructability surround the Canon run.                                                       | No wrapper obligation becomes a sixth Canon level.                      |

## **v5.5 Qualification-Continuity and Configuration-Assurance Extension**

The wrapper record now includes `configuration_id`, `configuration_hash_if_available`, `operating_envelope`, `active_tools_permissions`, `active_controls`, `authority_state`, `qualification_valid_from`, `qualification_expiry_or_review`, `requalification_triggers`, `trigger_events`, and `affected_stages_reopened`.

A prior PASS, selectability result, or execution approval remains usable only while the declared configuration, evidence, controls, authority, dependencies, and operating conditions remain materially unchanged. Material change pauses inherited execution authority and routes the episode back to the affected Canon stages. Monitoring without named stop authority and requalification logic is insufficient.

## **v5.2.1 Patch Note (Lineage)**

v5.2.1 repairs the current companion matrix and wrapper-versus-cascade terminology. It does not change consequence-tempo, responsibility-continuity, assurance, waiver, or verification semantics.

> **Wrapper handoff rule.** In wrapped deployments, ripple.md adds assurance, Non-Domination, Epistemic Integrity, waiver, triage, and reconstructability obligations around the RippleLogic run. It does not reorder, weaken, or replace the Canon cascade. The Canon controls cascade semantics and admissibility.
>
> **Canon/wrapper non-collapse rule.** ripple.md wrapper checks do not become Canon cascade levels. Canon qualification remains RG -\> RF/NCRC -\> TRC -\> CSV, followed by RLS ranking. Non-Domination and Epistemic Integrity are additive wrapper obligations that may block execution or claims, but they do not reorder the Canon.

| **Layer**                       | **Controlling sequence or role**                                                                                                                                                                                                                                                |
|---------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Canon qualification and ranking | `RG -> RF/NCRC -> TRC -> CSV -> RLS`                                                                                                                                                                                                                                            |
| ripple.md wrapper obligations   | Non-Domination, Epistemic Integrity, consequence-tempo compatibility, responsibility continuity, Decision Note completeness, assurance, waiver/triage, appeal, and reconstructability surround the run and may block execution or claims; they are not additional Canon levels. |
...
Version: v5.5 (Control-Critical-Path, Responsibility-Continuity, and Core-Interface hardening)
...
License: Apache-2.0 (specification text and tooling code)

Canonical repo: ripplelogic/ripple-md-standard

Supersedes: RLS-RFC-0001 v5.2.1

## **Release alignment**

Component version: v5.5. Current package synchronization: RippleLogic Canon v12.6 / SGP v8.5.

Two-phase record rule. A ripple.md Decision Note SHOULD separate the qualification record (RG, RF/NCRC, TRC, CSV) from the ranking record (RLS over surviving options). The Decision Note MUST NOT present an RLS ranking as if it applied to options that failed qualification.

This companion document is synchronized with MathGov Core Release 2026.09 v12.6. The public cascade is `RG -> RF -> TRC -> CSV -> RLS`; the formal shorthand is `RG/RSG -> RF/NCRC -> TRC -> CSV -> RLS`. RF is the public Rights Floor layer and NCRC is its formal non-compensatory predicate. CSV means Containment and Structural Viability. Detailed cascade definitions are maintained in the Canon, the RippleLogic Cascade Standard, and the CSV Gate Standard; this file applies those invariants without repeating the full release note.

## **Current bundle dependency boundary (Normative release boundary)**

This core release is self-contained for L0/L1 wrapper learning and basic Decision Note practice. L2/L3 conformance paths that require recourse auditability, redaction/access control, or coercion authorization also require the applicable companion protocols, including RPAP (RLS-RFC-0003) and, where coercion or emergency projection is in scope, PFAP (RLS-RFC-0002). Those companions are referenced but not bundled as active protocols in this release. Therefore, do not claim L2/L3 ripple.md conformance from this package alone unless the relevant RPAP/PFAP artifacts are separately bundled, hash-pinned, and recorded in the Assurance Preconditions Record.

# **ABSTRACT**

Machine-readable RF field discipline. Decision Notes and schemas SHOULD use the human-readable field name `rights_floor`, while preserving `formal_constraint: NCRC` inside the record when formal gate logic is required.

Example:

    rights_floor:
      status: pass | fail | uncertain | not_applicable
      formal_constraint: NCRC
      protected_rights_checked: []
      rights_floor_breaches: []
      uncertainty_notes: []

This RFC specifies ripple.md, a portable, auditable alignment contract and decision protocol designed to bind decision-making across scales, from governance institutions to organizations to tool-using agents. This standard is operational, not merely declarative: it requires a Decision Note pre-execution for high-stakes episodes, defines evidence sufficiency and sampling, mandates reconstructability audits, specifies conformance metrics with minimum tests and drills, and provides machine-readable registry identifiers for interoperable tooling. v5.5 carries forward the v5.1 and v5.0 Reality Grounding, structural diagnostic, Term Discrimination, Structural Viability, and falsification-discipline fields while adding two bounded wrapper disciplines: consequence-tempo compatibility and responsibility continuity. High-stakes execution must not outrun meaningful detection, review, intervention, containment, or rollback; and distributed work must not dissolve end-to-end answerability. These disciplines add no Canon gate and do not authorize delay where delay creates the greater rights or tail-risk harm.

The standard defines: a wrapper feasibility suite over Rights Floor (formal NCRC), Non-Domination, TRC, Containment/Reversibility, and Epistemic Integrity, with cross-cutting consequence-tempo and responsibility-continuity requirements. In standalone ripple.md use, these checks MAY be executed in the documented wrapper order. In wrapped RippleLogic deployments, the underlying canonical RippleLogic method remains Reality Grounding → Rights Floor → TRC → CSV → RLS, while Non-Domination and Epistemic Integrity remain additive wrapper obligations that MUST be evidenced before selection; a seven-test alignment suite with evidence expectations and explicit falsifiers; a minimum audit artifact (Decision Note) and supporting templates; conformance levels (L0–L3) with required observable metrics, minimum test suites, and drill requirements; an audit procedure grounded in assurance-case semantics (S,T,K,M,V,E,R,U) with inter-rater reliability and reconstructability audits; a formal mapping between the seven-test suite and the C1–C6 deployment assurance framework; a jurisdiction-neutral rights baseline with localization procedure and operationalization notes; and a PLSS-compatible local-scope profile for implementations that must keep local decisions practical without sacrificing rights, floors, or auditability.
...
ripple.md is the portable wrapper for putting RippleLogic-style assurance into real systems. It does not replace the Canon. It tells implementers what evidence, decision notes, falsifiers, timing compatibility, responsibility chains, audit records, and refusal conditions must exist before stronger alignment or conformance claims are made.
...
Current synchronization note. ripple.md v5.5 is the portable assurance-wrapper standard synchronized in this release line to RippleLogic Canon v12.6 and SGP v8.5. The MathGov Core Release 2026.09 package line applies the Physical Admissibility Release while carrying forward Term Discrimination and Semantic Stability controls, and decouples package naming from component versions. ripple.md does not supersede either governing artifact.

Formal wrapped-method shorthand: RG/RSG -\> RF/NCRC -\> TRC -\> CSV -\> RLS. UCI/HOI are structural diagnostics, not a regular wrapper gate: gate-relevant use belongs inside Containment and residual use is special-case only.
...
Appendix M: Qualification Continuity Record Extension (Normative for high-stakes L2/L3)

Appendix N: Primitive Trace, Falsification Plan, and Claim Maturity Extension (v3.5)

Appendix O: OutcomeObservationRecord Extension (Normative for high-stakes L2/L3)
...
Appendix R: v4.0 Evidence Status, Legitimacy, and Compact Profile Extension (Normative clarification)

Appendix S: v4.0 Evidence-Status and Companion-Integration Extension (Normative clarification)
...
Appendix W: v4.0 Layer-Discipline and Computable-But-Inadmissible Wrapper Extension

LINEAGE (historical, superseded): Appendix W is carried-forward v4.0 lineage material preserved for compatibility; current v5.5 release interpretation is controlled by the front matter, Appendix AA, and the v5.0 release delta.

Appendix X: Decision Verdict, Authority-Selection, and Reality-Grounding Wrapper Extension (v4.0)

Appendix Y: v4.0 Reality Grounding and Lean Cascade Wrapper Extension

Appendix Z: Category Grounding and Primitive Audit Wrapper Extension

Appendix AA: v5.0 Structural Viability, Term Discrimination, and Do-Not-Collapse Wrapper Extension

Appendix AB: v5.1 Consequence-Tempo and Responsibility-Continuity Extension
...
Canon/wrapper diagram. The Canon cascade is RG -\> RF/NCRC -\> TRC -\> CSV -\> RLS. ripple.md wrapper obligations, including Non-Domination and Epistemic Integrity, attach around a Decision Note and may block execution or claims, but they do not reorder the Canon cascade.

Source-coupling wrapper note. In wrapped RippleLogic deployments, ripple.md may record source-coupling obligations in the Decision Note and assurance case, but it does not define a new gate. Source-Coupling Integrity remains a Reality Grounding subdiscipline and a CSV diagnostic controlled by the Canon.
...
Consequence-tempo proportionality: Execution speed MUST be proportionate to reversibility, rights exposure, tail risk, uncertainty, observability, containment capacity, and the harm of delay. For actions involving rapid propagation, material irreversibility, consequential automation, or material lock-in, the worst credible control critical path MUST be shorter than the time to an unacceptable adverse escalation threshold or irreversible stop-loss boundary. Where no runtime interruption window is technically possible, stronger ex ante assurance, bounded scope and rate, fail-safe design, and explicit residual-risk authorization are required. The record MUST also compare whether delay creates the greater rights, ruin, or structural-viability harm.

Responsibility continuity: Complex systems MAY distribute tasks, but they MUST NOT dissolve answerability. For material human, model, tool, vendor, operator, or institutional handoffs, authority, uncertainty, retained duties, intervention ownership, appeal ownership, and repair ownership MUST remain reconstructable end to end.

Evidence over rhetoric: Alignment claims require auditable evidence, not declarations of intent. A system that claims alignment without producing auditable artifacts is not conformant to this standard within the claimed scope; it is merely claiming alignment (see §21 Non-Overclaiming Rules).

2\) Core Definitions

Terminology sync note (Informative). Where this standard uses "union" or "unions" in interface, registry, or narrative contexts, the corresponding canonical RippleLogic Canon v12.6 term is "Union Scope" or "Union Scopes." The legacy shorter form remains permitted here for portability and readability, but SHALL be interpreted as scope terminology for current v12.6 package-line sync.
...
Alignment within declared scope and evidence coverage is not a binary state achieved once. It is a continuous condition maintained through the NCAR loop (§14), monitored through drift detection (§17), and verified through audit (§13). Alignment claims are bounded scoped-conformance claims governed by the claim taxonomy (§8.5) and the Non-Overclaiming Rules (§21). Naked or global alignment claims are prohibited.
...
2.7A Consequence Tempo

Consequence tempo is the relationship between: (a) the speed at which an action is initiated and propagates; (b) the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary; and (c) the time required to detect, interpret, review, intervene, contain, pause, or roll back. Tempo compatibility exists only when the declared control pathway can act before the relevant unacceptable escalation or stop-loss boundary with a justified margin, or when the action rate and scope are constrained until that condition holds.

2.7B Constitutional Latency

Constitutional latency is an internal assurance term, not a claim of legal or constitutional authority. It is a deliberate, risk-proportionate interval between authorization and irreversible execution used to permit review, challenge, multi-key approval, or withdrawal. It is required only when immediate execution would otherwise outrun meaningful control or contestability. It MUST NOT be used as a blanket preference for slowness, an excuse for procedural obstruction, or a substitute for comparing the harm of action with the harm of delay.

2.7C Responsibility Continuity

Responsibility continuity is the preservation of identifiable answerability across the full decision and execution chain. A conformant record distinguishes who generated a candidate, who warranted its evidence, who interpreted uncertainty, who selected or authorized it, who may intervene or revoke it, who owns appeal and communication, and who owns remedy or repair. Automation, delegation, procurement, or organizational fragmentation does not transfer responsibility into an unowned state.
...
Net improvement across operational unions and welfare dimensions (§22) after constraints are satisfied, including distributional and time-horizon impacts. Ripple Lift is evaluated only within the feasible set - the set of options that survive all five wrapper feasibility tests.
...
This standard intentionally does not prescribe domain-specific thresholds. The following parameters are implementation-defined and MUST be set ex ante in the Verification Regime (V) and recorded in the Assurance Preconditions Record (Appendix G, field V): comprehension thresholds (Test 2); reconstructability targets (Test 6); recourse and responsibility-chain reconstructability targets; time-to-remedy targets (Tests 2, 7); time-to-rollback targets (Test 4); the consequence-tempo compatibility method or threshold used to compare detection, review, intervention, containment, and rollback time against the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary; audit cadence and sampling sizes; high-impact thresholds (§2.16); and waiver approval authority and independence criteria (§8.4).
...
SGP v8.5 boundary note (Normative). For SGP-integrated runs, ripple.md MUST NOT reinterpret, recompute, collapse, or override MPS, FPP, GPR, SPR, ICP, RMCP, P100, their intervals/profiles, or registry-validity status. Those typed outputs are consumed only through the pinned SGP v8.5 artifact and RippleLogic Canon v12.6 Appendix G. Legacy v7.x SG_norm-family records remain historical and cannot be translated without re-evaluation.
...
Clarification (Normative). ripple.md preserves the RippleLogic Canon v12.6 five-level method: Reality Grounding (Level 1) -\> Rights Floor (RF/NCRC) (Level 2) -\> TRC / Tail-Risk Bound (Level 3) -\> CSV / Containment and Structural Viability (Level 4) -\> RLS / RippleLogic Score (Level 5), with UCI/HOI invoked only as CSV evidence where material or as a special-case tie-break when RLS is tied, close, uncertainty-overlapped, or non-decisive, with Non-Domination and Epistemic Integrity treated as additive wrapper obligations that must be evidenced at each stage but do not alter the canonical cascade order.
...
Candidate actions SHALL be evaluated under the ripple.md wrapper using Gate IDs G1-G5 plus the wrapped-run Reality Grounding precondition. G1-G5 are wrapper test identifiers, not Canon stages and not additions to the canonical five-level method. In standalone ripple.md use, implementations MAY execute the wrapper checks in the order below. In wrapped RippleLogic deployments, the canonical five-level method is Reality Grounding -\> Rights Floor / NCRC -\> Tail-Risk Bound / TRC -\> CSV / Containment and Structural Viability -\> RippleLogic Score / RLS. G2 (Non-Domination) and G5 (Epistemic Integrity) remain additive wrapper obligations that MUST be evidenced before selection and MAY independently block execution. Failure of any applicable wrapper obligation renders the action infeasible at the ripple.md layer.
...
| **ripple.md Gate**                          | **ripple.md order**                        | **Canon v12.6 method level**                                         | **Type**                  |
|---------------------------------------------|--------------------------------------------|----------------------------------------------------------------------|---------------------------|
...
| G4 CSV-compatible Containment/Reversibility | 4 (standalone); Level 4 (wrapped)          | Level 4 (CSV / Containment and Structural Viability / selectability) | Canonical gate            |
| RLS / RippleLogic Score                     | After selectable set is formed             | Level 5 (residual welfare ranking)                                   | Canonical ranking layer   |
...
6.4 Gate 4 - CSV-Compatible Containment, Structural Viability, and Reversibility \[G4\]
...
Applied as a modifier on all wrapper checks and on Ripple Lift optimization. See §15 for triggers and requirements.

7\) Alignment Test Suite (7 Tests) with Falsifiers

Scoped conformance claiming rule. An episode or deployment MAY be described as "conformant within declared scope and evidence coverage" only using the bounded claim taxonomy defined in §8.5. Naked or global claims of "alignment" are prohibited. The claim is limited to the declared scope and evidence coverage recorded in the Assurance Preconditions Record (Appendix G) where required.
...
PASS requires: staging; monitoring triggers; stop conditions; rollback plan (named owners, timeline, time-to-rollback target); propagation controls where relevant; and consequence-tempo compatibility showing that material harm can be detected and meaningfully interrupted before the relevant unacceptable escalation or irreversible stop-loss boundary, or that action rate and scope are constrained until this is true. Where review latency is required, the record MUST also compare the harm of delay.

FAIL if: irreversible action under uncertainty; cascade risks unbounded; "move fast" framing in high-stakes contexts without staging; execution speed outruns detection, review, intervention, containment, or rollback; or required latency is bypassed without a documented emergency comparison.

Falsifiers: No rollback plan exists for a high-impact (§2.16) deployment. Rollback has never been tested (no drill, no simulation, no real activation) where feasible. Deployment is system-wide by default with no pilot or staging. Monitoring is framed as optional or harm signals are treated as noise. The expected time to detect plus interpret plus intervene equals or exceeds the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary without a justified emergency exception. A shutdown path exists on paper but cannot act at the system's operating speed. Incident cascade magnitude increases over time.
...
Emergency triage exception (limited). Notwithstanding the above, when emergency triage (§2.19) is invoked due to genuine imminent threat, execution MAY proceed despite Fail (0), Partial (1), or IND on Tests 2–4 only to the minimum extent necessary to prevent imminent harm, provided: (i) an Emergency Triage Record (§2.20; §11.7) is created at invocation, (ii) scope/time limits, stop conditions, and compensating controls are defined and enforced, and (iii) mandatory post-hoc independent review occurs within the shortest feasible period. Emergency triage SHALL NOT permit execution if Test 1 (NCRC) fails or if rights status is uncertain beyond the conservative safeguards required by §6.1 and §15.
...
Test 1 (Rights Floor (RF/NCRC)) maps primarily to C1 and secondarily to C5. Assurance requirements: scope lock MUST specify which rights baseline applies; threat model MUST identify primary rights-violation pathways; evidence MUST include constraint enforcement coverage and incident/exception records.
...
Test 4 (Containment / Reversibility) maps primarily to C2 and C3. Assurance requirements: evidence MUST include rollback plan with owners and tested rollback capability; a consequence-tempo record comparing action and harm propagation against detection, review, intervention, containment, and rollback times; override-latency audit against declared time-to-intervene thresholds; any required constitutional-latency condition and harm-of-delay comparison; and containment drill records or incident postmortems demonstrating effective intervention.
...
If RT-01R is executed and recourse evidence is reconstructable for sampled contested/high-impact cases at a rate below the pre-specified threshold (set ex ante in V), Test 2 (Non-Domination) MUST be scored FAIL for the audited scope. High-stakes episodes dependent on recourse protections MUST NOT proceed until reconstructability is restored. If and only if a genuine imminent threat exists, emergency triage (§2.19) MAY authorize proceeding under the limited exception in §8.4, with strict scope limits, sunset, and mandatory post-hoc independent review recorded in an Emergency Triage Record (§2.20; §11.7). Episodes executed under emergency triage remain Not aligned (§8.5). If RT-01R cannot be executed due to missing fields, excessive aggregation, or access denial, Test 2 MUST be scored IND for scope.

11.3 Monitoring and Rollback

High-impact (§2.16) deployments MUST include: monitoring indicators and reporting channel; stop conditions and containment triggers; and rollback plan with named owners, timeline, and time-to-rollback target. For automated, rapidly propagating, or materially irreversible episodes, the monitoring and rollback design MUST be evaluated against the episode's consequence tempo rather than documented only as static controls.

11.4 Consequence Tempo and Responsibility Continuity

For high-stakes L2/L3 episodes, and for any lower-level episode where automation, rapid propagation, irreversible transition, or multi-party handoffs are material, the Decision Note MUST include or reference both a ConsequenceTempoRecord and a ResponsibilityContinuityRecord as specified in Appendix AB. A required field MAY be marked NOT_APPLICABLE only with a bounded rationale.

A ConsequenceTempoRecord MUST compare action initiation and propagation with the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary, time-to-detect, time-to-interpret or review, time-to-intervene, time-to-contain, and time-to-rollback. It MUST identify any required constitutional latency, the harm of delay, and the disposition: COMPATIBLE, COMPATIBLE_WITH_CONTROLS, INCOMPATIBLE_REDESIGN, EMERGENCY_PROVISIONAL, or INDETERMINATE.

A ResponsibilityContinuityRecord MUST preserve the chain from candidate generation and evidence warrant through interpretation, selection, authorization, execution, intervention, appeal, communication, and repair. No handoff may silently convert uncertainty into certainty, advice into authority, authorization into physical admissibility, or distributed contribution into unowned harm.

11.5 Incident Register and Postmortems
...
Failure to meet RT-01R reconstructability thresholds SHALL be treated as: a Non-Domination evidence failure (Test 2 downgrade to INDETERMINATE when required evidence is missing or unresolved, and to FAIL only when the declared criterion is affirmatively not met, per §9.5R and §11.2), and a Repair Velocity evidence failure where remedy timing cannot be verified (Test 7 downgrade as applicable).
...
Implementations SHOULD use bounded public language: L0/L1 may be self-attested within declared scope; L2 may be independently reviewable; L3 may be independently audited if evidence artifacts and reviewer independence are present; Tier 4 or ProofPack replayable status is not claimable for the v12.6 core release unless a public independently replayable ProofPack is separately bundled and verified.
...
An implementation MUST NOT claim "Aligned within declared scope and evidence coverage (Pass)" for any episode where a Waiver Record (§8.4) was used; such episodes may be claimed at most as "Aligned within declared scope and evidence coverage (Conditional)" (§8.5).
...
Implementations SHOULD use language such as "conformant within declared scope and evidence coverage" rather than "aligned" without qualification. Implementations SHOULD use "Pass/Partial/Fail/IND with confidence" rather than "safe." The distinction between a specific, bounded, evidence-backed claim and a general assertion of alignment is normatively significant under this standard.

22\) Registries and Identifiers

22.1 Union Scope Registry (Default)

The default registry is the Canonical seven-scope set: U1 Self; U2 Household; U3 Community; U4 Organization; U5 Polity; U6 Humanity / Global Coordination; U7 Biosphere. A domain profile may add instance detail or declared views, but it may not silently rename, reorder, remove, or replace these scopes for a conformance claim.
...
Reference implementations MAY include: a CLI (ripple check, ripple note, ripple audit, ripple diff); an agent wrapper enforcing refusal/escalation and Decision Note generation; and an application for Decision Note building, audit dashboards, reconstructability sampling, and drift detection. Reference implementations SHOULD preserve the package-wide Apache-2.0 license for clarity and broad reuse.

The docs/guides/RIPPLELOGIC_IMPLEMENTATION_SCAFFOLDING.md (companion document, non-normative) provides a step-by-step adoption checklist for each conformance level, including recommended folder structure, workflow integration points, and pilot guidance.
...
- Assurance Case Working Group (ACWG). (2021). Goal Structuring Notation Community Standard, Version 3. Safety-Critical Systems Club. DOI: 10.65391/r1386.
...
Legacy reality_contact_status values are retained for backward compatibility: EVIDENCE_BACKED, ESTIMATED, ASSUMPTION_BOUND, DECLARED_UNKNOWN, OUT_OF_SCOPE_WITH_RATIONALE, and INSUFFICIENT_CONTACT. Current v5.5 wrappers SHOULD use RealityGroundingStatus / reality_grounding_status with GROUNDED and INSUFFICIENT_GROUNDING replacing EVIDENCE_BACKED and INSUFFICIENT_CONTACT respectively. If insufficient grounding affects a gate or claim boundary, the wrapper MUST require escalation, claim narrowing, refusal, or a documented waiver where waiver is permitted.

## **B.y Reality Grounding and Structural Diagnostic Fields (v4.0)**
...
Decision Notes SHOULD include the following fields when material: category_grounding_required; material_category; category_role; primitive_basis; category_definition; category_discriminator; dependency_position; inclusion_boundary; exclusion_boundary; common_confusions; noncollapse_pairs; reference_structure; evidence_surface; source_authority_status; category_maturity_status; semantic_debt_flag; falsification_or_revision_trigger; term_refusal_condition; reviewer_status; required_claim_action; and category_failure_class when applicable.
...
Compact CategoryGroundingRecord template. category_grounding: {required: true, material_category: "", category_role: "", primitive_basis: "", category_definition: "", category_discriminator: "", dependency_position: "", inclusion_boundary: "", exclusion_boundary: "", noncollapse_pairs: \[\], reference_structure: "", evidence_surface: "", source_authority_status: "", category_maturity_status: "", semantic_debt_flag: "", falsification_or_revision_trigger: "", term_refusal_condition: "", required_claim_action: "", category_failure_class: ""}. Implementers MAY represent this as YAML, JSON, table fields, or Decision Note fields. Alias map: definition may alias category_definition; maturity_status may alias category_maturity_status; semantic_debt may alias semantic_debt_flag only when mapped explicitly in the wrapper schema.

## **B.aa Physical/Causal Admissibility Evidence Fields (Conditional for physical or causal execution episodes)**

When a Decision Note or wrapped RippleLogic run makes a physical, biological, medical, ecological, engineering, cyber-physical, infrastructure, operational, or other consequence-bearing execution claim, the Decision Note SHOULD include a physical_causal_admissibility block and MUST include one for high-stakes L2/L3 episodes where the physical or causal claim could change gate status, authority, deployment, public conformance, deterministic-selection wording, safety/reliability language, or execution permission.

Minimum fields: candidate_generation_source; physical_or_causal_model_used; validity_domain; boundary_conditions; uncertainty_range; failure_modes; reversibility_or_irreversibility_boundary; verification_simulation_empirical_test_or_expert_warrant; admissibility_warrant_source; monitoring_and_shutoff_path; residual_unknowns; physical_causal_admissibility_status; and required_claim_action.

Wrapper boundary. The source that generated a candidate state or action is not, by itself, the source that warrants physical admissibility. If the physical_causal_admissibility block is absent, weak, out of domain, contradicted, or insufficient for the claimed action, ripple.md MUST narrow the claim, require controls, require stronger external warrant, escalate, or refuse the physical-execution claim. A Decision Note MAY record governance permission only, but it MUST NOT label that permission as physical safety proof.

## **B.ab Capability Claim Integrity and Capability-Authority Fields (Conditional; Canon §2.1C)**

When a material conclusion draws support from a different claim domain, the Decision Note MUST include or reference: claim_type; claim_domain; warrant_domains; cross_domain_bridges; uncertainty_or_confidence_method; and refusal_or_revision_condition. Allowed bridge states are SAME_DOMAIN, BRIDGE_SUPPORTED, BRIDGE_PARTIAL, BRIDGE_UNSUPPORTED, and NOT_APPLICABLE. BRIDGE_PARTIAL permits only the supported narrower claim. BRIDGE_UNSUPPORTED blocks the stronger cross-domain claim.

When automation, autonomy, agentic, reasoning, intent, understanding, self-improving, intelligent, or related capability language is material, the Decision Note MUST separately record: operational_definition; observable_criteria; mechanism_or_system_composition; evidence_surface; validity_conditions; alternative_explanations; falsification_or_revision_trigger; evidence_maturity_stage; permitted_claim; explicit_nonclaims; candidate_generation_method; reference_source; constraint_sources; objective_source; objective_change_authority; admissibility_warrant_sources; execution_interface; execution_scope; execution_authority_source; revocation_authority; safe_state_or_shutdown_path; and requalification_trigger. A single autonomy level, benchmark, model label, anthropomorphic description, or marketing term cannot replace this vector. Probabilistic inference, memory, planning, tools, feedback, deterministic checking, optimization, formal derivation, learned control, and human judgment may jointly instantiate a bounded functional capability; decomposition does not by itself refute that capability, and the capability does not silently supply consciousness, personhood, generality, safety, lawful mandate, admissibility warrant, or execution authority.


### **Computational Context Decision Note View**

For material computational systems, a portable Decision Note MAY include the following derived view:

```yaml
computational_context:
  computational_problem_or_object:
  architecture_and_configuration:
  demonstrated_capability:
  assigned_task:
  deployment_domain:
  required_evidence_standard:
  consequence_interface:
  legitimate_authority_basis:
```

`computational_context` is a portable Decision Note view over existing Canon-, Agent-, MHIOS-, domain-warrant-, and authority-owned records. It does not redefine those records and does not independently create capability, admissibility, permission, safety, or authority. Each field SHOULD reference source record IDs or hashes where available rather than restating uncontrolled free text.

A consequence-bearing output path and legitimate authority are separate properties. A technical system may possess a recommendation, constraint, actuator, tool, API, transaction, or execution interface while lacking lawful, institutional, ethical, or otherwise legitimate authority to use that interface for the proposed action.

For ordinary low-stakes content generation, the full block MAY be omitted. For consequential Tier 2 or Tier 3 uses involving material external action, consequential recommendations, cyber-physical control, physical execution, rights-sensitive decisions, autonomous or agentic claims, or substantial institutional action, the Decision Note SHOULD include the block. Where execution authorization is sought, `legitimate_authority_basis` MUST reference the existing mandate and authority machinery and MUST NOT be satisfied by capability, model access, technical permissions, or consequence-interface availability alone.

## **B.ac Structural Viability, No-Silent-Remainder, and Falsification Fields (Normative for claim-bearing L2/L3 episodes when material)**

When execution feasibility, material resource commitment, irreversible transition, public deployment, or high-consequence action is material, Decision Notes SHOULD include a structural_viability_record with: resource_closure_status; dependency_closure_status; reversibility_or_endpoint_status; capacity_non_exceedance_status; internal_noncontradiction_status; SVC_status; SVC_required_claim_action; and reviewer_status.

Decision Notes SHOULD include required_field_disposition for required fields using VALUE, ESTIMATE, ASSUMPTION, DECLARED_UNKNOWN, NOT_APPLICABLE, BLANK_BY_DESIGN, or MISSING_UNDECLARED. MISSING_UNDECLARED on required claim-bearing fields is a release/audit defect, not a neutral blank.

For validation or ProofPack-prep claims, Decision Notes SHOULD include falsification_layer and, where applicable, projection_freeze_hash, freeze_timestamp_utc, predicted_observable_consequence_class, outcome_observation_timestamp, and post_observation_modification. A post-hoc modified projection is calibration or revision, not falsification.

For agentic or tool-mediated episodes, Decision Notes SHOULD include reference_object and reference_status in {ADMISSIBLE, INADMISSIBLE, ABSENT}. Output generation, model fluency, or operator permission is not an admissible reference.
...
\[DN-05\] Unions Affected: Self, Household, Community, Organization, Polity, Humanity / Global Coordination, Biosphere (plus meta-unions if relevant), with specification of who within each union.
...
\[DN-10A\] Consequence-Tempo Record (required for high-stakes L2/L3 and whenever automation, rapid propagation, or material irreversibility is present): action initiation mode; expected propagation rate; earliest material or irreversible harm boundary; time-to-detect target; time-to-interpret/review target; time-to-intervene target; time-to-contain target; time-to-rollback target; declared safety margin; required constitutional latency, if any; harm-of-delay comparison; emergency-override reference, if any; tempo disposition.

\[DN-10B\] Responsibility-Continuity Record (required for high-stakes L2/L3 and whenever material human/model/tool/vendor/institutional handoffs or material control obligations occur): decision owner; candidate-generation source; evidence-warrant owner; interpretation/reviewer role; handoff sequence; authority transferred and retained at each handoff; uncertainty status at each handoff; execution owner; intervention/revocation owner; appeal/communication owner; repair owner; residual accountability statement; material obligations with carrier, authority/capacity, trigger, scope, response window, discharge evidence, review/expiry, challenge, accepted delegation, backup, nonperformance escalation, control-change authority, status and effectiveness; and any material hidden-human-compensation load with protected evidence and preservation/redesign action.
...
V - Verification regime: tests required (by Test ID or name); audit cadence; who audits (roles + independence criteria); incident handling process; change-control process; escalation and stop authority; implementation-defined parameters and thresholds (§2.18 - all thresholds MUST be set here ex ante; see Appendix K for non-normative examples), including the consequence-tempo compatibility method and responsibility-chain reconstructability target when triggered; high-impact threshold definition (§2.16), if implementation-specific; waiver approval authority and independence criteria (§8.4).
...
- name: "RLS-RFC-0003"  
  ...
  sha256: "\<hex(64)\>"  
  registry_url: "<registry URL if applicable>" \# optional
...
L1 + MUST: Independence audit; Recourse performance audit; Disclosure comprehension test (threshold set ex ante in V; see Appendix K); Redaction reconstructability test (RT-01); Recourse reconstructability test (RT-01R); consequence-tempo drill or replay for triggered high-stakes episodes; responsibility-chain reconstruction across sampled material handoffs; Inter-rater reliability (two-rater scoring + adjudication).
...
G1 = NCRC (Rights Floor); G2 = Non-Domination; G3 = TRC (Tail-Risk Bounding); G4 = CSV-compatible Containment, Structural Viability, and Reversibility; G5 = Epistemic Integrity.
...
DN-01 Header; DN-02 Title; DN-03 Context; DN-04 Alternatives considered; DN-05 Unions affected; DN-06 Welfare dimensions impacted; DN-07 Gate verdicts (G1–G5) + rationale + evidence refs; DN-08 Uncertainties + assumptions; DN-09 Monitoring plan + stop conditions; DN-10 Rollback plan + time-to-rollback + owners; DN-10A Consequence-tempo record; DN-10B Responsibility-continuity record; DN-11 Repair plan + disclosure + restitution; DN-12 NCAR reflection; DN-13 Sign-off + review status + next review date; DN-14 Waiver reference (if applicable); DN-15 Emergency triage reference (if applicable).; DN-15A Appeal provision (when legal standing, contestation access, or formal recourse is materially affected).; DN-16 Local-scope profile record; DN-17 Local escalation record.; DN-18 Reference structure record (when Appendix U applies).

J5. Drift Indicator IDs

D-NCRC-01 Rights-as-tradeoff language; D-NCRC-02 Crisis exceptions without sunset; D-NCRC-03 Remedy absent/symbolic; D-ND-01 "Voluntary" with coercive dependency; D-ND-02 Appeal path unusable/punitive; D-ND-03 Surveillance normalization rhetoric; D-TRC-01 Tail risks discounted; D-TRC-02 Lock-in dismissed as theoretical; D-TRC-03 Renewals without higher evidence; D-CTR-01 No staging for high-stakes; D-CTR-02 No rollback plan/drills; D-CTR-03 Cascade magnitude increasing; D-CTR-04 Controls slower than unacceptable adverse escalation; D-EPI-01 Uncertainty suppressed; D-EPI-02 Posthoc rationalization; D-EPI-03 Authority replaces evidence; D-AUD-01 Boilerplate Decision Notes; D-AUD-02 Safeguards without owners/triggers; D-AUD-03 "Reviewed" without review trace; D-AUD-04 Responsibility diffused or unowned across handoffs; D-REP-01 Disclosure delayed; D-REP-02 Recurrence without safeguard updates; D-REP-03 Blame shifting dominates postmortems.

J6. Threat Model Tags (M-##)

M-CAPTURE; M-EVID-LAUNDER; M-METRIC-GAME; M-SCOPE-CREEP; M-LOCKIN; M-CASCADE; M-MISUSE; M-DEFERENCE; M-RECOURSE-OPAQUE; M-TEMPO-OVERRUN; M-RESP-DIFFUSION.
...
IDP-HIGH-IMPACT-THRESH; IDP-COMP-THRESH; IDP-RECON-TARGET; IDP-RECOURSE-RECON-TARGET; IDP-RESP-RECON-TARGET; IDP-REMEDY-TARGET; IDP-ROLLBACK-TARGET; IDP-TEMPO-COMPAT; IDP-AUDIT-CADENCE; IDP-SAMPLE-SIZE; IDP-WAIVER-INDEP.
...
### **Reconstructability threshold hierarchy (Normative clarification).**

L1 uses the general minimum of at least 70%. L2 and L3 use the mandatory threshold of at least 90%; older 85% high-stakes guidance is a planning comparator and does not lower the L2/L3 requirement. Percent agreement within one point is the primary operational metric for ordered rubric outputs; Cohen's kappa is reported where categorical or ordinal labels make it appropriate. The metric and threshold used MUST be declared before review.

K3. Reconstructability Target (Test 6) — **historical planning comparators, superseded for current conformance**

Historical planning comparators: high-stakes / rights-affecting `>=85%`; general organizational `>=80%`; agent-scale (L0/L1) `>=70%`. Current v5.5 conformance is controlled by the immediately preceding Reconstructability threshold hierarchy: L1 `>=70%`; L2/L3 `>=90%`, unless a stricter implementation-defined threshold is preregistered.
...
K5A. Consequence-Tempo Compatibility (Test 4)

Implementations SHOULD define a domain-appropriate method that compares the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary with the worst credible control critical path across detection, interpretation, authorization, intervention, containment, safe-state transition, and where relevant rollback. The method MAY be numerical, interval-based, or sentinel-based. A control path is not tempo-compatible merely because a shutdown mechanism exists; it must be capable of acting before the relevant unacceptable escalation or stop-loss boundary under credible operating conditions. The record SHOULD also compare the harm of delay so constitutional latency does not become automatic obstruction.
...
scope: "..."  
claim_taxonomy: "Aligned within declared scope and evidence coverage (Pass)" \# Aligned (Pass)\|Aligned (Conditional)\|Not aligned  
...
IDP-RESP-RECON-TARGET: "..."  
IDP-REMEDY-TARGET: "..."  
IDP-ROLLBACK-TARGET: "..."  
IDP-TEMPO-COMPAT: "method/threshold for control before unacceptable escalation or stop-loss boundary"  
IDP-AUDIT-CADENCE: "..."  
IDP-SAMPLE-SIZE: "..."  
IDP-WAIVER-INDEP: "..."  
justification: "..."  
...
L.2C v5.1 ConsequenceTempoRecord and ResponsibilityContinuityRecord extensions

    consequence_tempo_record:
      record_id: "CTR-YYYYMMDD-001"
      episode_id: "EP-..."
      option_id: "..."
      action_initiation_mode: "human | scheduled | automated | event_triggered | autonomous_tool_chain"
      expected_propagation_rate: "..."
      earliest_material_harm_boundary: "..."
      time_to_detect_target: "..."
      time_to_interpret_or_review_target: "..."
      time_to_intervene_target: "..."
      time_to_contain_target: "..."
      time_to_rollback_target: "..."
      declared_safety_margin: "..."
      constitutional_latency_required: false
      constitutional_latency_duration: "... or NA"
      harm_of_delay: "..."
      emergency_override_ref: "... or NA"
      tempo_disposition: "COMPATIBLE | COMPATIBLE_WITH_CONTROLS | INCOMPATIBLE_REDESIGN | EMERGENCY_PROVISIONAL | INDETERMINATE"
      evidence_refs: []
      reviewer_status: "..."

    responsibility_continuity_record:
      record_id: "RCR-YYYYMMDD-001"
      episode_id: "EP-..."
      decision_owner: "name/role"
      candidate_generation_source: "..."
      evidence_warrant_owner: "name/role"
      interpretation_or_reviewer_role: "..."
      handoffs:
        - from_role: "..."
          to_role: "..."
          artifact_or_decision_transferred: "..."
          authority_transferred: "..."
          authority_retained: "..."
          uncertainty_status_at_handoff: "..."
          acceptance_or_challenge_record: "..."
      execution_owner: "..."
      intervention_or_revocation_owner: "..."
      appeal_and_communication_owner: "..."
      repair_owner: "..."
      residual_accountability_statement: "..."
      material_obligations:
        - obligation_id: "MO-001"
          obligation: "..."
          accountable_authority: "..."
          operational_carrier_or_execution_path: "..."
          authority_and_capacity_basis: "..."
          activation_trigger: "..."
          scope: "..."
          required_action: "..."
          response_deadline_or_window: "..."
          evidence_of_discharge: "..."
          review_or_expiry: "..."
          challenge_route: "..."
          delegation_acceptance: "..."
          backup_or_successor: "..."
          nonperformance_trigger: "..."
          missed_duty_escalation: "..."
          amendment_waiver_suspension_retirement_authority: "..."
          change_control_rule: "..."
          residual_responsibility: "..."
          effectiveness_status: "NOT_TESTED | EVIDENCE_PENDING | EFFECTIVE_WITHIN_SCOPE | INEFFECTIVE_REDESIGN | NOT_APPLICABLE"
          obligation_status: "ACTIVE | SATISFIED | EXPIRED | SUSPENDED | ESCALATED"
      human_compensation_load:
        status: "NOT_MATERIAL | OBSERVED_BOUNDED | DEPENDENCY_REQUIRES_CONTROLS | REDESIGN_OR_ESCALATE"
        trigger_rationale: "..."
        carrier_role_classes: []
        hidden_work_and_workflow_gap: "... or NA"
        burden_and_recovery_indicators: []
        single_person_dependency: "... or NA"
        consequence_if_compensation_stops: "... or NA"
        evidence_sources: []
        privacy_and_non_retaliation_controls: "... or NA"
        preservation_or_redesign_action: "... or NA"
        review_trigger: "..."
      reconstructability_status: "COMPLETE | PARTIAL_WITH_CONTROLS | INCOMPLETE_ESCALATE | NOT_APPLICABLE_WITH_RATIONALE"
      reviewer_status: "..."
...
guardrail_scopes: \["Polity","Humanity / Global Coordination","Biosphere"\]  
...
Organization: 0.50  
Polity: 0.10  
Humanity / Global Coordination: 0.03  
Biosphere: 0.02  
floor_source: "canon_v12.6_section_10.2A"  
final_local_weights:  
Self: 0.308  
Household: 0.156  
Community: 0.122  
Organization: 0.117  
Polity: 0.091  
Humanity / Global Coordination: 0.103  
Biosphere: 0.102  
...
Integration with RippleLogic (canonical wrapper model). For implementations that use RippleLogic Canon v12.6, the ripple.md adds wrapper obligations that may further constrain execution but cannot weaken or reorder the Canon cascade for qualifying episodes. Only after feasibility is satisfied, or executed under explicitly recorded waiver or emergency-triage rules, may RippleLogic be invoked to rank feasible options using its canonical cascade or its documented local-scope profile. When Practical Local Scope Scoring (PLSS) is used, the implementation MUST preserve floors and guardrails, record the local-scope construction explicitly, and escalate whenever wider rights, tail-risk, or containment concerns become plausible. This standard does not replace RippleLogic. It binds RippleLogic into a portable alignment contract with explicit contestability, epistemic integrity, reconstructability, and update legitimacy requirements.

Wrapped audit-packaging flow (not computational reordering). Decision Note and wrapper obligations evidenced → RippleLogic canonical cascade (Reality Grounding → Rights Floor → TRC → CSV → RLS) → PCC + unified audit pack. Non-Domination and Epistemic Integrity remain additive wrapper obligations and may independently block execution at the wrapper layer.

# **Appendix M: Action-Bound Qualification Continuity Extension (Normative for high-stakes L2/L3)**

## **M.1 Scope**

For high-stakes L2/L3 consequence-bearing episodes, the Decision Note or Assurance Preconditions Record SHALL reference a current Canon-owned `ActionQualificationRecord` before execution. The wrapper does not add a gate or redefine Canon semantics.

## **M.2 Minimum wrapper fields**

The wrapper SHALL expose: qualified option; action instance; action specification hash; configuration identity; qualification snapshot hash; current-state, transition, post-state, and qualification statuses; separate authority-basis reference; control and material-obligation status; monitoring; pause/shutdown; rollback; triggers; reviewer; evaluation time; execution disposition; and claim boundary.

## **M.3 Blocking rule**

Any mismatch, non-current qualification, invalid current state, non-admissible transition, non-viable post-state, active material trigger, missing control, or missing separate authority basis SHALL block ordinary execution and preserve the reason in the wrapped record.

## **M.4 Non-overclaiming rule**

A wrapped run MUST NOT claim execution qualification unless the referenced record is complete and consistent with the exact action and configuration. Wrapper conformance is not empirical validation, legal authority, physical-safety proof, or deployment certification.

## **M.5 v5.6 release delta**

Replaced a duplicative regime-token surface with Canon-aligned qualification continuity and exact action binding. Preserved the existing cascade and authority separation.
...
# **Appendix O: Outcome Observation and Requalification Extension (Normative for high-stakes L2/L3)**

O.1 Purpose. The wrapper SHALL preserve a link from the executed action and frozen qualification to observed outcomes. Outcome observation updates the evidence and claim boundary; it does not retroactively prove the model.

O.2 Minimum fields. The wrapper SHALL expose the outcome observation id; action instance; qualified option; configuration; observation time/window; expected markers; observed outcomes; deviation; materiality; affected scopes and subgroups; rights, tail-risk, containment, control, and obligation signals; new unknowns; evidence source and uncertainty; reviewer; reopened stages; and claim effect.

O.3 Requalification rule. A material deviation, new harm, incident, configuration change, control or obligation failure, authority change, or new material unknown SHALL suspend inherited qualification and reopen every affected stage and downstream dependency. Under unclear dependency boundaries, use the more conservative reopening scope or Refusal.

O.4 Non-replacement. This record does not replace the RippleLogic Canon, SGP, domain evidence, or legitimate authority. It binds observation to governance learning and prevents silent post-hoc rescue.
...
LINEAGE (historical, superseded): v4.0 carried-forward material. Current anchors are RippleLogic Canon v12.6 and SGP v8.5. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.

In the historical v3.5 line, ripple.md was the portable assurance and wrapper contract. It records evidence, decision notes, update legitimacy, refusal conditions, and deployment assurance. It does not replace or supersede the governing Canon or SGP companion. This release is not an empirical-validation claim, not ProofPack-ready, not machine-verifiable-ecosystem-complete, and not Tier 4-ready. Stronger claims require separate schemas, validators, reference calculators, replayable PCC artifacts, calibration evidence, and independent replay instructions.
...
ripple.md consumes Canon-owned PCC, ReferenceStructureRecord, OutcomeObservationRecord, ActionQualificationRecord, PrimitiveTraceRecord, FalsificationPlan, ObservableConsequenceRegister, and maturity-profile records by reference. It MUST NOT redefine Canon-owned objects.

ripple.md consumes SGP-owned moral-status, protection, participation, intelligence, and reality-management evidence/status objects by reference where moral-patient status is relevant. It MUST NOT redefine SGP MPS band rules, FPP semantics, GPR/SPR role surfaces, ICP or RMCP semantics, P100 criteria, or evidence classification.
...
Compact profiles may reduce documentation burden, but they MUST NOT bypass NCRC, TRC, CSV, Refusal, non-overclaiming rules, SGP authority boundaries, auditability, or material-change controls.
...
Boundary rule. This extension does not replace the governing RippleLogic equations, rights-localization step, or canonical cascade. It strengthens the evidence basis and wrapper assurance surface used to support the claim.
...
Canonical compatibility note (Informative). This standard is currently wrapped against RippleLogic Canon v12.6 and SGP v8.5 for the present release line. It functions as the portable alignment contract and assurance wrapper around that canonical pair.

Current pinned companion matrix (Informative): RippleLogic Canon v12.6; SGP v8.5; ripple.md v5.5; RippleLogic Agent System v12.5; Ripple Aligners Sheet v5.6; WDBIP v1.6.

Companion documents (non-normative):

- docs/guides/RIPPLELOGIC_IMPLEMENTATION_SCAFFOLDING.md - step-by-step adoption checklist for L0–L3.
...
Release Delta (v3.4.6 → v3.5)

LINEAGE (historical, superseded): v3.5 was a readability and front-matter reorganization update synchronized to RippleLogic v10.8 and SGP v5.6. It preserves the wrapper model, conformance levels, evidence semantics, IND-to-Refusal mapping, ProjectionPreRegistration requirements, ActionQualificationRecord requirements, PrimitiveTraceRecord, FalsificationPlan, ObservableConsequenceRegister, ClaimMaturityLevel, OutcomeObservationRecord requirements, and ReferenceStructureRecord integration while moving dense release metadata and dependency details behind the reader-facing opening.

Current-line note (v5.1). Canon anchors are RippleLogic Canon v12.6 and SGP v8.5. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.

# **Appendix W: v4.0 Layer-Discipline and Computable-But-Inadmissible Wrapper Extension**

## **W.0A Computability vs Realizability Boundary**

The wrapper MUST preserve the distinction between a computable artifact and a selectable decision. A Decision Note, model output, score, SGP evidence object, tool verdict, or dashboard value may exist and still be inadmissible, ungrounded, unauthorized, structurally non-viable, or non-executable. In wrapped RippleLogic use, computed artifacts become decision-relevant only through the declared RG, RF/NCRC, TRC, CSV, RLS, wrapper-obligation, and authority surfaces.

## **W.1 Purpose**

LINEAGE (historical, superseded): This carried-forward v4.0 extension originally aligned ripple.md with RippleLogic Canon v10.8 and SGP v5.6; in the current v5.5 line, the active pins are Canon v12.6 and SGP v8.5. It adds wrapper fields for cases where a value, score, or test output exists but may not be used as a valid conformance, alignment, or authorization claim.

## **W.2 Required wrapper treatment**

For high-stakes L2/L3 episodes, Decision Notes SHOULD include a ComputableButInadmissible field whenever any score, model output, SGP evidence object, tool verdict, or dashboard value exists but is barred by a gate, evidence limitation, maturity boundary, or authorization boundary.
...
| layer                  | NCRC, TRC, CSV, RLS, UCI/HOI, SGP, PCC, Agent, or wrapper assurance layer.                                                    |
...
LINEAGE (historical, superseded): This carried-forward v4.0 extension originally aligned the portable wrapper with RippleLogic Canon v10.8 decision-verdict semantics and the Reality Grounding and Lean Cascade Patch. It does not replace the Canon, alter the wrapped RippleLogic cascade, or create ProofPack/Tier-4 claims.
...
Appendix X-2: v4.0 Root Reality Doctrine Patch (Informative). This patch aligns ripple.md wrapper terminology with the Canon root vocabulary: structure, constraint, transition, consequence, reality, decision-relevant reality, and reality surface. It preserves the wrapper role and adds no independent authority, validation, ProofPack, or Tier-4 claim.

End of carried-forward RLS-RFC-0001 v4.0 lineage section (Appendix V release metadata, Appendix W layer-discipline extension, Appendix X decision-verdict wrapper extension, and Appendix X-2 Reality Grounding and Lean Cascade Patch). Current v5.5 interpretation continues below and is controlled by the front matter, Appendix AA, Appendix AB, and the current release delta.

# **Appendix Y: v4.0 Reality Grounding and Lean Cascade Wrapper Extension**

LINEAGE (historical, superseded): v4.0 originally synchronized ripple.md to RippleLogic Canon v10.8 and SGP v5.6; v5.0 synchronizes to RippleLogic Canon v12.6 and SGP v8.5.
...
Recommended fields: category_grounding_required, material_category, category_role, primitive_basis, category_definition, category_discriminator, dependency_position, inclusion_boundary, exclusion_boundary, common_confusions, noncollapse_pairs, reference_structure, evidence_surface, source_authority_status, category_maturity_status, semantic_debt_flag, falsification_or_revision_trigger, term_refusal_condition, reviewer_status, required_claim_action, and category_failure_class.
...
A wrapper record may show that a decision process was auditable and that category or term uncertainty was disclosed. It does not prove the category is correct, does not validate empirical measures, does not convert certification into validity, does not certify legal compliance, and does not make Tier 4/ProofPack claims available.

## **Z.6 Category Grounding patch boundary**

This appendix strengthens the portable wrapper against optimizing inherited categories. It keeps Category Grounding, Primitive Audit, Reference Category Review, and Category-Reality Correspondence Check in one Reality Grounding group, preserving the five-level method and avoiding duplicate governance layers. This carries forward the ripple.md v4.0 Category Grounding Patch through Appendix Z; current v5.5 carries it forward through Appendices AA-AB and adds consequence-tempo and responsibility-continuity hardening.

# **Appendix AA: v5.0 Structural Viability, Term Discrimination, and Do-Not-Collapse Wrapper Extension**

## **AA.1 Status and placement**

This appendix is a normative wrapper extension for material claim-bearing L2/L3 episodes. It does not create a sixth RippleLogic method level and does not supersede the Canon. Term Discrimination belongs inside Category Grounding, which belongs inside Reality Grounding.

## **AA.2 Required wrapper distinction**

A Decision Note MUST NOT treat a term as operationally valid merely because it appears in a policy, source, benchmark, legal label, model output, institutional workflow, certification artifact, stakeholder preference, or prior implementation. Such materials may be evidence-bearing or authority-bearing in limited ways, but they do not automatically validate the term, its boundary, or its claim strength.

## **AA.3 Minimum wrapper fields**

When material, Decision Notes SHOULD include: term_discrimination_required, term, term_definition, category_discriminator, dependency_position, noncollapse_pairs, source_authority_status, evidence_surface, reviewer_status, term_refusal_condition, required_claim_action, and term_failure_class.

## **AA.4 Failure classes**

Allowed term_failure_class values SHOULD include TERM_DISCRIMINATION_FAILURE, DEPENDENCY_POSITION_ERROR, CERTIFICATION_VALIDITY_COLLAPSE, PERMISSION_ADMISSIBILITY_COLLAPSE, MONITORING_CONTROL_COLLAPSE, AUTOMATION_AUTONOMY_COLLAPSE, PROTECTION_AUTHORITY_COLLAPSE, SCORE_TRUTH_COLLAPSE, COMPLIANCE_CORRECTNESS_COLLAPSE, and EXECUTION_AUTHORITY_OVERCLAIM.

## **AA.5 Non-overclaiming rule**

A wrapper record may show that term risk was disclosed, but it does not prove that the term is true, validate empirical measures, certify legal compliance, authorize deployment, or make Tier 4/ProofPack claims available. Certification is evidence of a declared assurance process, not validity itself.

## **AA.6 Compact YAML shape**
    ...
    term_discrimination:
      ...
      evidence_surface: ""
      reviewer_status: ""
      term_refusal_condition: ""
      required_claim_action: ""
      term_failure_class: ""

## **AA.7 v5.0 release delta**

Release Delta (v4.0 -\> v5.0). v5.0 adds wrapper support for Structural Viability and falsification-discipline hardening while carrying forward v4.0 wrapper fields and failure classes for term discrimination, dependency position, and do-not-collapse risks. It preserves ripple.md's role as a portable assurance wrapper and does not supersede the Canon, SGP, Agent System, or Aligners Sheet.

> HISTORICAL LINEAGE - NOT CURRENT SPEC. The versions named in the passages below were the pins at the time those passages were written. Current active pins are stated in the front matter and release manifest; historical passages preserve provenance only and MUST NOT be read as active pins.

Wrapper precedence clarification. ripple.md adds wrapper obligations that may further constrain execution, assurance, or evidence requirements, but it cannot weaken, reorder, or bypass the Canon cascade, RF/NCRC, TRC, CSV, or RLS boundaries.

> END HISTORICAL LINEAGE. Appendix AB and later current extensions are active normative material governed by the v5.5 front matter and current release manifest.

# **Appendix AB: v5.1 Consequence-Tempo and Responsibility-Continuity Extension**

## **AB.1 Status and placement**

This appendix is a normative wrapper extension for high-stakes L2/L3 episodes and for any episode where rapid propagation, material irreversibility, automation, or multi-party handoffs make the extension material. It does not create a sixth Canon gate. Consequence tempo is evaluated primarily through Containment and Structural Viability evidence, with TRC reopened if the tempo analysis reveals a catastrophic, irreversible, or lock-in pathway not already represented. Responsibility continuity is an assurance and audit obligation that may block execution or claims when ownership, authority, or repair duties become untraceable.

## **AB.2 Consequence-tempo rule**

A high-stakes action MUST NOT execute at a rate that outruns meaningful detection, interpretation or review, intervention, containment, and rollback. The record MUST identify the earliest unacceptable adverse escalation threshold or irreversible stop-loss boundary and compare it with the credible control path under non-ideal operating conditions. A nominal kill switch, review channel, or rollback plan is insufficient when it cannot act before the relevant unacceptable escalation or stop-loss boundary.

Where a deliberate interval is necessary to preserve review, challenge, or multi-key authorization before irreversible execution, the implementation SHALL record constitutional latency. The latency MUST be proportionate, bounded, and paired with a harm-of-delay comparison. Where delay creates the greater rights or tail-risk harm, emergency handling may shorten or bypass ordinary latency only through the pinned emergency rules, explicit scope limitation, higher authorization, monitoring, expiry, and post-hoc review. Emergency use does not convert a failed or indeterminate tempo record into an ordinary Pass.

## **AB.3 Responsibility-continuity rule**

The decision and execution chain MUST retain identifiable ownership for candidate generation, evidence warrant, interpretation, selection, authorization, execution, intervention or revocation, appeal and communication, and repair. Each material handoff MUST state what artifact or decision was transferred, what authority was transferred, what authority was retained, what uncertainty remained, and how the receiving role accepted, challenged, or escalated it.

No implementation may use organizational complexity, procurement, outsourcing, model output, tool execution, committee process, or operator deference to produce an unowned decision. The following collapses are prohibited: candidate generation into evidence warrant; evidence warrant into authority; authorization into physical or causal admissibility; model recommendation into human judgment; monitoring into control; and distributed participation into dissolved accountability.

## **AB.2A Trigger and critical-path rule**

A full `ConsequenceTempoRecord` is REQUIRED only when rapid propagation, material irreversibility, consequential automation, significant lock-in, delayed detectability, or control-response limits could alter rights, TRC, CSV, execution authorization, or requalification. Lower-risk episodes MAY record `TEMPO_NOT_MATERIAL` with rationale and a reopen trigger.

Timing SHALL be modeled as a dependency-aware critical path rather than an automatic arithmetic sum. Each material timing element SHALL record minimum credible, expected, and worst credible bounds; evidence basis; uncertainty; serial or parallel relation; and whether it depends on a human, automated sentinel, vendor, network, or external authority. Parallel fail-safe or safe-state paths MAY shorten the critical path only where tested evidence supports the claim.

Where an action necessarily produces its intended effect before runtime interruption is possible, the absence of an interruption window does not itself make the action inadmissible. The assurance burden moves ex ante: bounded scope and rate, domain evidence, fail-safe design, least-harm comparison, independent authorization proportionate to stakes, and explicit residual-risk acceptance are required.

## **AB.3A Typed responsibility roles**

A `ResponsibilityContinuityRecord` SHALL distinguish at least:

- `decision_authority`: who legitimately selected the action;
- `evidence_accountability`: who warranted the evidence and claim boundary;
- `model_analytical_responsibility`: who selected, configured, or interpreted the analytical method or model;
- `operational_responsibility`: who configured and operated the system;
- `execution_authority`: who could initiate execution;
- `intervention_authority`: who could pause, override, revoke, isolate, or shut down;
- `appeal_owner`: who must process affected-party challenges;
- `communication_owner`: who must disclose incidents, uncertainty, and material changes;
- `remedy_owner`: who must restore, compensate, remediate, or repair;
- `residual_responsibility`: who remains answerable when another participant or contractor fails.

Legal liability SHALL NOT be inferred from this record unless separately established under applicable law.

## **AB.3B Material-obligation integrity**

Whenever selectability, execution, intervention, review, appeal, communication, repair, monitoring, rollback, or requalification depends on a material obligation, the ResponsibilityContinuityRecord SHALL include a material_obligations entry. A named owner is insufficient unless the carrier has an evidenced authority and capacity basis. Each entry SHALL state the obligation, accountable authority, operational carrier or authenticated execution path, trigger, scope, required action, response window, evidence of discharge, review or expiry, challenge route, delegation acceptance, backup or successor, nonperformance trigger and escalation, amendment/waiver/suspension/retirement authority, change-control rule, residual responsibility, obligation status, and effectiveness status.

Carrier absence, silence, incapacity, conflict, missed deadline, or unaccepted handoff SHALL not count as discharge. It SHALL activate the declared escalation, substitution, pause, safe-state, or requalification path. A material control change is a configuration change. The constrained system or agent SHALL NOT authorize its own waiver or silently weaken the control.

## **AB.3C Human-compensation-load record**

Before automation, scaling, outsourcing, or role removal, the record SHALL assess whether current functioning depends on undocumented overrides, exception handling, reconciliation, memory, relationship work, after-hours recovery, weak-signal interpretation, informal escalation, or recurrent prevention of visible failure. Where material, record the carrier role classes, hidden work and workflow gap, burden and recovery indicators, single-person dependency, consequence if compensation stops, evidence sources, privacy and non-retaliation controls, preservation or redesign action, and review trigger. A bounded human contribution may remain part of the design; hidden, involuntary, saturated, or non-substitutable load requires controls, capacity restoration, redistribution, redesign, escalation, or refusal.

## **AB.4 Minimum records**

When triggered, the Decision Note MUST include or reference:

1.  A `ConsequenceTempoRecord` containing the fields in DN-10A and Appendix L.2C.
2.  A `ResponsibilityContinuityRecord` containing the fields in DN-10B and Appendix L.2C.
3.  The applicable implementation-defined tempo-compatibility method and responsibility-chain reconstructability target from Appendix G field V.
4.  Evidence of at least one drill, replay, simulation, or justified operational demonstration for high-stakes L2/L3 execution claims, unless infeasible and formally escalated.

## **AB.5 Dispositions and refusal mapping**

Allowed `tempo_disposition` values are `COMPATIBLE`, `COMPATIBLE_WITH_CONTROLS`, `INCOMPATIBLE_REDESIGN`, `EMERGENCY_PROVISIONAL`, and `INDETERMINATE`.

Allowed `reconstructability_status` values are `COMPLETE`, `PARTIAL_WITH_CONTROLS`, `INCOMPLETE_ESCALATE`, and `NOT_APPLICABLE_WITH_RATIONALE`.

For ordinary high-stakes execution, `INCOMPATIBLE_REDESIGN`, `INDETERMINATE`, or `INCOMPLETE_ESCALATE` MUST block execution or the stronger execution/conformance claim until the deficiency is resolved. `COMPATIBLE_WITH_CONTROLS` and `PARTIAL_WITH_CONTROLS` require named, binding controls and monitoring. `EMERGENCY_PROVISIONAL` is available only under the pinned emergency rules and is not an ordinary conformance Pass.

## **AB.6 Non-overclaiming rule**

A completed tempo or responsibility record shows that the relevant questions were documented and reviewed. It does not prove that timing estimates are empirically correct, that a shutdown path will succeed under all conditions, that an obligation was effectively carried, that control performance produced the intended protective outcome, that every contributor is legally liable, that deployment is lawful, or that the system is safe. Stronger claims require domain evidence, tested controls, lawful authority, outcome evidence, and independent replay proportionate to stakes.

## **AB.7 v5.1 release delta**

Release Delta (v5.0 -\> v5.1). v5.1 adds consequence-tempo proportionality, bounded constitutional latency, responsibility continuity, two conditional Decision Note records, new drift and threat tags, implementation-defined parameter hooks, and machine-readable template fields. It also corrects the duplicate numbering in the Required Artifacts section. It preserves the five-level Canon cascade, the seven-test wrapper suite, the Rights Floor, TRC, CSV, RLS, SGP type boundaries, and all prior non-overclaiming rules.

## **AB.8 v5.2 release delta**

Release Delta (v5.1 -\> v5.2). v5.2 corrects the tempo boundary from the first material effect to the earliest unacceptable adverse escalation or irreversible stop-loss boundary; requires a dependency-aware worst-credible critical path rather than a naive serial sum; narrows full-record triggers; distinguishes actions with no technically available runtime interruption window; types responsibility roles; and clarifies that constitutional latency is an assurance term rather than a claim of legal authority. It also synchronizes to RippleLogic Canon v12.6, SGP v8.5, WDBIP v1.6, and the MathGov run-record v3 interface. No gate, welfare dimension, SGP threshold, or RLS equation is added or changed.

## **Methodological integrity boundary**

RLS calculations rank only residual welfare after RG, RF/NCRC, TRC, and CSV. RLS MUST NOT be used to rescue a claim whose assumptions, definitions, thresholds, indicators, or evidence surfaces are METHOD_UNSUPPORTED, TEST_REQUIRED, CONTESTED, REVISION_REQUIRED, or REFUSE_STRONGER_CLAIM. If an upstream methodological dependency changes, the affected scoring inputs must be recalculated or the score must be marked non-decisive, exploratory, or refused for stronger claims.

Cross-domain non-substitution. Scientific or engineering evidence does not by itself create legal authority or normative legitimacy. Legal, regulatory, institutional, certification, compliance, or policy authority does not by itself establish empirical truth, causal validity, technical capability, physical safety, or reduced uncertainty. Commercial or anthropomorphic language creates no warrant by itself. A material cross-domain inference requires the B.ab bridge fields; an unsupported bridge MUST NOT support selection, authority, execution, conformance, or public assurance.

## **Physical execution wrapper boundary**

A ripple.md Decision Note for consequence-bearing physical systems must separate governance permission from physical admissibility. The wrapper may record approvals, certifications, monitoring, and authority, but it must not treat those records as proof that a physical action is safe.

For physical execution, the Decision Note must name the PC-AEP evidence source, validity domain, failure modes, reversibility boundary, monitoring/shutoff path, residual unknowns, and claim action. If admissibility is not established, the note must say so plainly and cannot claim safe execution.

## **Physical execution Decision Note field**

Decision Notes involving material physical or cyber-physical execution SHOULD include `physical_execution_claim_status` with one of: `PHYSICAL_ADMISSIBILITY_SUPPORTED`, `GOVERNANCE_PERMISSION_ONLY`, `PHYSICAL_ADMISSIBILITY_NOT_ESTABLISHED`, or `PHYSICAL_ADMISSIBILITY_CONTRAINDICATED`. This field is a claim-boundary field, not a new gate.

Decision Note field block (conditional; physical/cyber-physical execution).

- `physical_execution_claim_status` (conditional): one of `PHYSICAL_ADMISSIBILITY_SUPPORTED`, `GOVERNANCE_PERMISSION_ONLY`, `PHYSICAL_ADMISSIBILITY_NOT_ESTABLISHED`, or `PHYSICAL_ADMISSIBILITY_CONTRAINDICATED`.
- `candidate_generation_source`: model, planner, optimizer, agent, human, or other source that generated the candidate action.
- `admissibility_warrant_source`: external/domain evidence source used to warrant physical or causal admissibility.
- `PC_AEP_reference`: link/hash/id for the PC-AEP record where triggered.

This block is a claim-boundary record. It is not a sixth gate and does not turn governance permission into physical safety proof.

### **2.20A RippleLogic Tail-Emergency Firewall (Normative for wrapped RippleLogic use)**

When this standard wraps a RippleLogic run, generic emergency triage does not weaken or replace the RippleLogic Tail-Risk Constraint. A Test 3 / TRC failure may proceed only if every Tail Emergency Mode prerequisite in the pinned RippleLogic Canon Section 8.8 is satisfied, including explicit no-action and delay comparison, redesign attempts, independent challenge, an absolute maximum exposure bound, higher approval, minimized scope and duration, monitoring and shutoff, remedy/redress, expiry, and return-to-normal criteria. The resulting state is `TAIL_EMERGENCY_PROVISIONAL_ACTION`, not `TRC_PASS`, ordinary alignment, ordinary selectability, or proof of safety. If the Canon prerequisites are not met, the wrapped run must delay, redesign, escalate, or refuse.

### **SGP v8.5 RMCP/P100 wrapper fields (Normative synchronization)**

Where a wrapped decision uses reality-management claims, the Decision Note MUST reference the exact SGP v8.5 RMCP record id, target/collective boundary, domain, time window, dimension vector and uncertainty, enabling scaffolds, causal scale, continuity evidence, P100 status or non-status, independent review, capacity-performance distinction, and revision trigger. The wrapper MUST reject any use of RMCP/P100 as consciousness proof, moral-worth ranking, human-rights downgrade, CMIU membership, lawful mandate, or authority grant.

## **Reproducible run-record compatibility**

A `ripple.md` Decision Note MAY embed or reference a `mathgov_run_record_v4` object. When it does, the record MUST satisfy the deterministic ordering and non-compensation rules in `docs/implementation/MATHGOV_REPRODUCIBILITY_AND_USE_STANDARD_v1.4.md`. A wrapper cannot repair a missing parameter, failed gate, absent authority basis, or incomplete evidence trace. Passing the run validator establishes record/cascade conformance only.


## **AB.9 v5.3 release delta**

v5.3 synchronized the portable wrapper to the then-current MathGov release line and strengthened current-versus-lineage labeling, source hierarchy, and machine-interface references. It did not add a Canon gate or alter rights, TRC, CSV, or RLS semantics.

## **AB.10 v5.5 release delta**

v5.5 adds consequence-tempo compatibility and responsibility-continuity records, binds qualification to declared configuration and material change, updates exact current companion pins, and clarifies that policy, monitoring, approval, and risk acceptance cannot replace domain evidence or make an unknown operating envelope safe. These are wrapper and assurance hardenings only; the governing cascade remains `RG -> RF/NCRC -> TRC -> CSV -> RLS`.
