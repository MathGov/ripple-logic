...
- In standalone ripple.md use, wrapper gates MAY be executed in the documented wrapper order. In wrapped RippleLogic use, preserve the canonical RippleLogic cascade order and treat Non-Domination and Epistemic Integrity as additive wrapper obligations.
...
Current synchronization note. ripple.md v3.5.4 is retained as the portable assurance-wrapper standard and is synchronized in this release line to RippleLogic Canon v10.7.4 and SGP v5.4.4. The MathGov / RippleLogic Core Release 2026.06 package line applies the root reality doctrine patch and decouples package naming from component versions. ripple.md does not supersede either governing artifact.
...
6. Constraint Cascade (Normative Decision Ordering)
...
Appendix R: v3.5 Evidence Status, Legitimacy, and Compact Profile Extension (Normative clarification)
Appendix S: v3.5 Evidence-Status and Companion-Integration Extension (Normative clarification)
...
Appendix W: v3.5 Layer-Discipline and Computable-But-Inadmissible Wrapper Extension
Appendix W is part of the v3.5 standard release-clean line and is included in the completed v3.5 release boundary.
Appendix X: Decision Verdict, Authority-Selection, and Reality-Contact Wrapper Extension (v3.5.4)
## X.1A Reality-contact wrapper rule
For wrapped RippleLogic deployments, FrameworkVerdict and AuthoritySelectionRecord MUST be interpreted alongside RealityContactStatus. A non-decisive or insufficiently evidenced reality surface cannot be laundered into deterministic framework selection by adding an authority record.
0) Normative Keywords
...
2) Core Definitions
Terminology sync note (Informative). Where this standard uses "union" or "unions" in interface, registry, or narrative contexts, the corresponding canonical RippleLogic v10.7.4 term is "Union Scope" or "Union Scopes." The legacy shorter form remains permitted here for portability and readability, but SHALL be interpreted as scope terminology for current v10.7.4 package-line sync.
...
SGP boundary note (Normative). For SGP-integrated runs, ripple.md MUST NOT reinterpret, recompute, or override SG_norm(E), SG_patient_norm(E), SG_measured_norm(E), or Plateau(E). Those terms are consumed only through the pinned SGP artifact and the RippleLogic v10.7 Appendix G binding interface.
...
Clarification (Normative). ripple.md preserves the RippleLogic v10.7 lexicographic cascade: NCRC (Level 1) → TRC (Level 2) → Containment (Level 3) → RLS/Selection (Level 4) → UCI/HOI structural tie-break (Level 5, invoked only when RLS is non-decisive), with Non-Domination and Epistemic Integrity treated as additive wrapper obligations that must be evidenced at each stage but do not alter the canonical cascade order.
...
6) Constraint Cascade (Normative Decision Ordering)
Candidate actions SHALL be evaluated under the ripple.md wrapper using Gate IDs G1–G5. In standalone ripple.md use, implementations MAY execute these checks in the order below. In wrapped RippleLogic deployments, the underlying canonical RippleLogic order remains NCRC → TRC → Containment → RLS; G2 (Non-Domination) and G5 (Epistemic Integrity) remain additive wrapper obligations that MUST be evidenced before selection and MAY independently block execution. Failure of any applicable wrapper obligation renders the action infeasible at the ripple.md layer.
...
Implementations SHOULD use bounded public language: L0/L1 may be self-attested within declared scope; L2 may be independently reviewable; L3 may be independently audited if evidence artifacts and reviewer independence are present; Tier 4 or ProofPack replayable status is not claimable for the v10.7 core release unless a public independently replayable ProofPack is separately bundled and verified.
...
plss_episode_extension:
  ...
  local_prominence_signals:
    ...
    Humanity/CMIU: 0.03
    Biosphere: 0.02
  floor_source: "foundation_v10.7_section_10.2A"
  final_local_weights:
    Self: 0.308
    Household: 0.156
    Community: 0.122
...
Synchronized current-line pins to RippleLogic v10.7 and SGP v5.4.
...
# Appendix Q: Machine-Readable Wrapper Roadmap and Schema Ownership Boundaries
Current-line note (v3.5). Canon anchors are RippleLogic v10.7 and SGP v5.4. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.
...
Canonical compatibility note (Informative). This standard is currently wrapped against RippleLogic v10.7.4 and SGP v5.4.4 for the present release line. It functions as the portable alignment contract and assurance wrapper around that canonical pair.
Current pinned companion matrix (Informative): RippleLogic Canon v10.7.4; SGP v5.4.4; ripple.md v3.5.4; RippleLogic Agent System v10.7.4; Ripple Aligners Sheet v3.5.5.
...
v3.5 is a readability and front-matter reorganization update synchronized to RippleLogic v10.7 and SGP v5.4. It preserves the wrapper model, conformance levels, evidence semantics, IND-to-Refusal mapping, ProjectionPreRegistration requirements, ExecutionStabilityRecord requirements, PrimitiveTraceRecord, FalsificationPlan, ObservableConsequenceRegister, ClaimMaturityLevel, TransitionAdmissibilityRecord requirements, and ReferenceStructureRecord integration while moving dense release metadata and dependency details behind the reader-facing opening.
Current-line note (v3.5). Canon anchors are RippleLogic v10.7 and SGP v5.4. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.


# Appendix W: v3.5 Layer-Discipline and Computable-But-Inadmissible Wrapper Extension
## W.1 Purpose
This v3.5 extension aligns ripple.md with RippleLogic Canon v10.7 and SGP v5.4. It adds wrapper fields for cases where a value, score, or test output exists but may not be used as a valid conformance, alignment, or authorization claim.
...
Appendix X: Decision Verdict, Authority-Selection, and Reality-Contact Wrapper Extension (v3.5.4)
X.1 Purpose
This v3.5.4 extension aligns the portable wrapper with RippleLogic Canon v10.7.4 decision-verdict semantics and the root reality doctrine patch. It does not replace the Canon, alter the wrapped RippleLogic cascade, or create ProofPack/Tier-4 claims.
...
Appendix X-2: v3.5.4 Root Reality Doctrine Patch (Informative). This patch aligns ripple.md wrapper terminology with the Canon root vocabulary: structure, constraint, transition, consequence, reality, decision-relevant reality, and reality surface. It preserves the wrapper role and adds no independent authority, validation, ProofPack, or Tier-4 claim.
End of RLS-RFC-0001 v3.5.4 (normative standard, current-line wrapper addenda, Appendix V release metadata, Appendix W layer-discipline extension, Appendix X decision-verdict wrapper extension, and Appendix X-2 root reality doctrine patch).

<!-- Table 1 extracted from DOCX -->
| ripple.md Gate | ripple.md order | Canon v10.7 cascade level | Type |
| G1 NCRC | 1 | Level 1 (admissibility) | Canonical gate |
| G2 Non-Domination | 2 (standalone); additive wrapper (wrapped) | Not in cascade | Wrapper obligation |
| G3 TRC | 3 (standalone); Level 2 (wrapped) | Level 2 (admissibility) | Canonical gate |
| G4 Containment & Reversibility | 4 (standalone); Level 3 (wrapped) | Level 3 (selectable) | Canonical gate |
| G5 Epistemic Integrity | 5 (standalone); additive wrapper (wrapped) | Not in cascade | Wrapper obligation |

<!-- Table 2 extracted from DOCX -->
