...
## Tail Emergency Mode validation

A run in which every rights-safe option exceeds the TRC corridor must not automatically select the least-CVaR option. Validation must test whether redesign, delay, refusal, and no-action were considered; whether inaction is demonstrably worse; whether an independent challenger reviewed alternatives; whether an absolute maximum exposure is declared; and whether the provisional action has time bounds, shutoff, remedy, and exit criteria. Ordinary RLS must remain disabled in this state.
