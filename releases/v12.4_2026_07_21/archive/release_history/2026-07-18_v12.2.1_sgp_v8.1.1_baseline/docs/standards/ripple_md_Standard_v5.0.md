# RLS-RFC-0001: The ripple.md Standard v5.0
...
| ripple.md wrapper obligations | Non-Domination, Epistemic Integrity, Decision Note completeness, assurance, waiver/triage, appeal, and reconstructability surround the run and may block execution or claims; they are not additional Canon levels. |
...
Version: v5.0 (RF Clarity, Structural Viability, and Falsification Discipline wrapper update)
...
Supersedes: RLS-RFC-0001 v4.0

## Release alignment

Component version: v5.0. Current package synchronization: RippleLogic Canon v12.2.1 / SGP v8.1.1.
...
Current synchronization note. ripple.md v5.0 is retained as the portable assurance-wrapper standard and is synchronized in this release line to RippleLogic Canon v12.2.1 and SGP v8.1.1. The MathGov Core Release 2026.09 package line applies the Physical Admissibility Release while carrying forward Term Discrimination and Semantic Stability controls, and decouples package naming from component versions. ripple.md does not supersede either governing artifact.
...
Legacy reality_contact_status values are retained for backward compatibility: EVIDENCE_BACKED, ESTIMATED, ASSUMPTION_BOUND, DECLARED_UNKNOWN, OUT_OF_SCOPE_WITH_RATIONALE, and INSUFFICIENT_CONTACT. Current v5.0 wrappers SHOULD use RealityGroundingStatus / reality_grounding_status with GROUNDED and INSUFFICIENT_GROUNDING replacing EVIDENCE_BACKED and INSUFFICIENT_CONTACT respectively. If insufficient grounding affects a gate or claim boundary, the wrapper MUST require escalation, claim narrowing, refusal, or a documented waiver where waiver is permitted.
...
LINEAGE (historical, superseded): Appendix W is carried-forward v4.0 lineage material preserved for compatibility; current v5.0 release interpretation is controlled by the front matter, Appendix AA, and the v5.0 release delta.
...
Terminology sync note (Informative). Where this standard uses "union" or "unions" in interface, registry, or narrative contexts, the corresponding canonical RippleLogic Canon v12.2.1 term is "Union Scope" or "Union Scopes." The legacy shorter form remains permitted here for portability and readability, but SHALL be interpreted as scope terminology for current v12.2 package-line sync.
...
SGP v8.1.1 boundary note (Normative). For SGP-integrated runs, ripple.md MUST NOT reinterpret, recompute, collapse, or override MPS, FPP, GPR, SPR, ICP, RMCP, P100, their intervals/profiles, or registry-validity status. Those typed outputs are consumed only through the pinned SGP v8.1.1 artifact and RippleLogic Canon v12.2.1 Appendix G. Legacy v7.x SG_norm-family records remain historical and cannot be translated without re-evaluation.
...
Candidate actions SHALL be evaluated under the ripple.md wrapper using Gate IDs G1-G5 plus the wrapped-run Reality Grounding precondition. In standalone ripple.md use, implementations MAY execute the wrapper checks in the order below. In wrapped RippleLogic deployments, the canonical five-level method is Reality Grounding -> Rights Floor / NCRC -> Tail-Risk Bound / TRC -> CSV / Containment and Structural Viability -> RippleLogic Score / RLS. G2 (Non-Domination) and G5 (Epistemic Integrity) remain additive wrapper obligations that MUST be evidenced before selection and MAY independently block execution. Failure of any applicable wrapper obligation renders the action infeasible at the ripple.md layer.
...
| ripple.md Gate                              | ripple.md order                            | Canon v12.2 method level                                             | Type                      |
| ------------------------------------------- | ------------------------------------------ | -------------------------------------------------------------------- | ------------------------- |
...
Implementations SHOULD use bounded public language: L0/L1 may be self-attested within declared scope; L2 may be independently reviewable; L3 may be independently audited if evidence artifacts and reviewer independence are present; Tier 4 or ProofPack replayable status is not claimable for the v12.2 core release unless a public independently replayable ProofPack is separately bundled and verified.
...
Biosphere: 0.02  
floor_source: "foundation_v12.1_section_10.2A"  
final_local_weights:  
Self: 0.308  
Household: 0.156  
Community: 0.122  
...
Synchronized current-line pins to RippleLogic Canon v12.2.1 and SGP v8.1.1.
...
LINEAGE (historical, superseded): v4.0 carried-forward material. Current anchors are RippleLogic Canon v12.2.1 and SGP v8.1.1. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.
...
Canonical compatibility note (Informative). This standard is currently wrapped against RippleLogic Canon v12.2.1 and SGP v8.1.1 for the present release line. It functions as the portable alignment contract and assurance wrapper around that canonical pair.

Current pinned companion matrix (Informative): RippleLogic Canon v12.2.1; SGP v8.1.1; ripple.md v5.0; RippleLogic Agent System v12.0.1; Ripple Aligners Sheet v5.2.1.
...
Current-line note (v5.0). Canon anchors are RippleLogic Canon v12.2.1 and SGP v8.1.1. This standard remains the portable alignment contract and assurance wrapper around that canonical pair and does not supersede their governing roles.
...
LINEAGE (historical, superseded): This carried-forward v4.0 extension originally aligned ripple.md with RippleLogic Canon v10.8 and SGP v5.5; in the current v5.0 line, the active pins are Canon v12.2 and SGP v8.1.1. It adds wrapper fields for cases where a value, score, or test output exists but may not be used as a valid conformance, alignment, or authorization claim.
...
End of carried-forward RLS-RFC-0001 v4.0 lineage section (Appendix V release metadata, Appendix W layer-discipline extension, Appendix X decision-verdict wrapper extension, and Appendix X-2 Reality Grounding and Lean Cascade Patch). Current v5.0 interpretation continues below and is controlled by the front matter, Appendix AA, and the v5.0 release delta.

# Appendix Y: v4.0 Reality Grounding and Lean Cascade Wrapper Extension

LINEAGE (historical, superseded): v4.0 originally synchronized ripple.md to RippleLogic Canon v10.8 and SGP v5.5; v5.0 synchronizes to RippleLogic Canon v12.2.1 and SGP v8.1.1.
...
> HISTORICAL LINEAGE - NOT CURRENT SPEC. Current active component versions are: Canon v12.2 / SGP v8.1.1 / ripple.md v5.0 / Agent v12.0 / Primer v4.0 / Aligners Sheet v5.2.1 / Cascade Standard v2.2.1 / RLS Validation Protocol v2.2.1. Historical passages preserve provenance only and MUST NOT be read as active pins.
...
### SGP v8.1.1 RMCP/P100 wrapper fields (Normative synchronization)

Where a wrapped decision uses reality-management claims, the Decision Note MUST reference the exact SGP v8.1.1 RMCP record id, target/collective boundary, domain, time window, dimension vector and uncertainty, enabling scaffolds, causal scale, continuity evidence, P100 status or non-status, independent review, capacity-performance distinction, and revision trigger. The wrapper MUST reject any use of RMCP/P100 as consciousness proof, moral-worth ranking, human-rights downgrade, CMIU membership, lawful mandate, or authority grant.

## Reproducible run-record compatibility

A `ripple.md` Decision Note MAY embed or reference a `mathgov_run_record_v1` object. When it does, the record MUST satisfy the deterministic ordering and non-compensation rules in `docs/implementation/MATHGOV_REPRODUCIBILITY_AND_USE_STANDARD_v1.0.md`. A wrapper cannot repair a missing parameter, failed gate, absent authority basis, or incomplete evidence trace. Passing the run validator establishes record/cascade conformance only.
