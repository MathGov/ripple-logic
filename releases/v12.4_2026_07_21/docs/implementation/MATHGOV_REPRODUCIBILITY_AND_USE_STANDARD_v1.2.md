# MathGov Reproducibility and Use Standard v1.2


## v1.2 Current Release Integration
...
**Status:** Normative implementation companion for RippleLogic Canon v12.4. The Canon controls semantics and equations if a conflict occurs. This standard controls the minimum run record, deterministic cascade order, parameter-completeness rules, replay procedure, and proportional-use profiles for this release.
...
| K07 | Form the selectable set only from options supported by RG and passing RF/NCRC, holding a qualifying TRC state (`TRC_PASS` or documented `TRC_NOT_TRIGGERED`), and passing CSV. | `NO_SELECTABLE_OPTION` if empty |
...
5. `CSV_PASS`, `CSV_PASS_WITH_CONTROLS`, `CSV_NOT_MATERIAL`, `CSV_FAIL`, `CSV_REDESIGN_REQUIRED`, or `CSV_ESCALATE`
...
Run-record v2 uses the canonical state registry and transition matrix. `CSV_REDESIGN_REQUIRED` is the emitted token. The legacy input alias `CSV_REDESIGN` MAY be accepted only during the declared migration window and SHALL be normalized with a warning.
...
The machine-readable minimum is defined by `schemas/mathgov_run_record_v2.schema.json`. A human-readable run MUST expose the same logical fields even when it uses prose or a workbook.
...
1. RippleLogic Canon v12.4 controls decision semantics and equations.
2. SGP v8.3 controls MPS/FPP/GPR/SPR/ICP/RMCP/P100 semantics.
