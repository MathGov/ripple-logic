...
## Source-Coupling Integrity and related external work

MathGov's Source-Coupling Integrity diagnostic is a native Reality Grounding and CSV hardening rule. It asks whether a claimed capability remains traceable to its enabling conditions and boundary conditions before the run relies on downstream output, fluency, compliance, permission, or inherited procedure.

A related external concept appears in Alexander Wolfgang Strüver's 2026 paper, *Source Amnesia: How Civilizations Lose Their Understanding of Generative Structures*. That work is cited as related scholarship only. MathGov does not adopt that title as a MathGov module, does not reproduce or adapt the paper, and does not bundle it into the Apache-licensed release.
